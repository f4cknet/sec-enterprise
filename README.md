...
《企业安全指南》是笔者根据自身的安全从业经验编写，从`检测、防御、响应、合规`4个维度出发，涉及到`应用安全、数据安全、基础、应急响应、办公安全、安全合规`6个域。
![](https://m1nzhi.oss-cn-beijing.aliyuncs.com/secinstruc.png)

## 目的
本指南的定位是帮助中小型互联网企业(非安全企业)快速构建、完善`检测、防御、响应、合规`体系，不涉及具体的攻击性技术研究，对于非安全型的中小型互联网企业而言，攻击性技术研究过于奢侈，所以本指南不过多阐述攻击性技术。

## 适用目标
- 如果你所在的公司还在业务试错期那么请忽略本指南，先把重心放到业务上。
- 如果你所在的公司业务很多、规模很大，那么我相信本指南也无法满足你们的安全需求，所以也请忽略。
所以，该指南适合的对象是介于两者之间，比如已经有了明确业务方向的、初具规模的创业公司。有明确的业务方向和一定的业务规模这2点是关键。

## 邀请协作
笔者是乙方渗透出身，转甲方4年，主要工作经验在`应用安全`和`数据安全`，虽然在工作中也接触过其它的安全域，但毕竟不是专业的，比如安全合规、办公安全。所以希望有更多专业的安全人员加入编写、完善该指南，使之成为一个优秀的安全项目。
...
2. 私下交流：通过`infosecnote`公众号发我私信、或者发送邮件到zmzsg100@gmail.com和我交换私人微信。

## 修订记录
| 修订日期 | 修订内容 | 修订人 | 修订人联系 | 
| ----   | ----    |  ---- | ---- |
| 2023.9.17 | 完成“关于本指南”模块的编写| m1nzhi| zmzsg100@gmail.com |
...
## 通过风险评估进行安全成本估算
该方法优势：能通过风险估算可能造成的损失，然后根据可能这些损失来计算安全投入。如果说通过业务规模是定性分析，那么该方法就是定量分析，依托风险损失计算公式的值进行对应的安全投入，有所依据，对老板而言更有说服力。

劣势：定量风险评估公式中的暴露因子EF和年度威胁发生率没有明确数据参考，需要做大量的调研才能估算出这2个值。

度量安全ROI的第一步是进行风险评估，业内有很多风险评估指南，但步骤大致都相同
![](https://m1nzhi.oss-cn-beijing.aliyuncs.com/fengxianpinggu.png)

## 安全成本计算公式
```
SLE=Asset value * EF（单次风险造成的损失）
ALE=SLE * ARO（一年总风险造成的损失）
AROI=invest-ALE（一年投资回报值）
```

定量计算安全成本的难点在于EF需要安全专家进行主动性判断。

这里我举个例子来说明下大概如何定义EF值，比如现在很流行的越权问题，假如某接口存在敏感信息且存在越权。这里有2种情况，一种是数据ID可自增可遍历，那么EF就是1，因为攻击者可以通过遍历获取所有id的敏感数据。如果ID是无法遍历的随机数，那么EF就可以定在0.1-0.5之间。
而关于资产的定级和估值，安全工程师需要找业务方去沟通，虽然现在等保对大部分业务系统都定为三级，但其实有很大的差异。有些三级系统可能用户量只有几万人，有些则有上百万人。只有和业务方沟通，才能更准确的对业务系统的价值进行评估。
比如拥有50万-100万用户的业务系统估值100万，超过100万用户的业务系统估值500万。当然用户数量并不是唯一参数，业务系统的属性是很重要的参考指标，比如 {% u 变现能力很强的业务 %} 估值可以（用户数量 * 2），而 {% u 变现能力稍微差点 %} 的业务则（用户数 * 0.5）。

每年的安全投入成本则是上面公式中的ARO * 0.3 ~ ARO * 0.7比较合理。

| 资产分类 | 资产       | 资产估值    | 威胁   | 单一损失期望  | 年度发生率 | 年度损失期望值 | 总和     | 安全成本预算 | 安全成本投入点               |
|------|----------|---------|------|---------|-------|---------|--------|--------|-----------------------|
| 业务系统 | xx业务系统   | 1000000 | 数据泄漏 | 1000000 | 0.15  | 150000  | 177000 | 88500  | 业务系统检测产品、防御产品、软件安全工程师 |
|      | xx内部运营平台 | 500000  | 业务中断 | 200000  | 0.01  | 2000    |        |        |                       |
|      | xxx业务系统2 | 200000  | 直接资损 | 500000  | 0.05  | 25000   |        |        |                       |
| 涉密文件 | 机密文件1    | 500000  | 数据泄漏 | 500000  | 0.5   | 250000  | 800000 | 400000 | 数据防泄漏、数据安全治理          |
|      | 机密文件2    | 100000  | 数据泄漏 | 100000  | 0.5   | 50000   |        |        |                       |
|      | 核心业务源代码  | 1000000 | 数据泄漏 | 1000000 | 0.5   | 500000  |        |        |                       |
